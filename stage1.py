# =========================
# STAGE 1 (2 CLASSES, SEMI-HARD TRIPLET MINING)
...
# PATHS
# =========================
ROOT = "datasets/stage1"

TRAIN_DIR = "data/train1"
VAL_DIR = "data/val1"
TEST_DIR = "data/test1"

SUPPORT_DIR = "data/support_set1"
QUERY_DIR = "data/query_set1"

OUTPUT = "outputs/predicted_output_stage1"
MODEL_PATH = "models/model_1.pth"           # state_dict
MODEL_FULL_PATH = "models/model_1_full.pth" # full model
...
# =========================
# SUPPORT / QUERY FROM TEST (NO LEAKAGE)
# =========================
def create_support_query():
    ...
    for cls in ["G", "P"]:
        ...
        support_imgs = imgs[:K_SUPPORT]
        query_imgs = imgs[K_SUPPORT:]
...
class ImgDataset(Dataset):
    def __init__(self, root, train=False):
        self.train = train
        self.data = []
        for label, cls in enumerate(["G", "P"]):
...
def train_model():
    ...
    plt.grid(True)
    # Title removed intentionally for publication style
    plt.savefig("outputs/loss_curve_stage1.png", dpi=300)
    plt.close()
    ...
    print("\nSaved Stage‑1 full model:", MODEL_FULL_PATH)
    print("Saved Stage‑1 state_dict:", MODEL_PATH)
...
def evaluate(model):
    ...
    # =========================
    os.makedirs("models", exist_ok=True)
    joblib.dump(prototypes, "models/prototypes_stage1.pkl")
    joblib.dump(scaler, "models/scaler_stage1.pkl")

    print("\nSaved Stage‑1 Prototypes: models/prototypes_stage1.pkl")
    print("Saved Stage‑1 Scaler: models/scaler_stage1.pkl")
    ...
    for p, pred in zip(paths, preds):
        folder = "Ganwar" if pred == 0 else "Paddy"
        os.makedirs(f"{OUTPUT}/{folder}", exist_ok=True)
        shutil.copy(p, f"{OUTPUT}/{folder}/{os.path.basename(p)}")

    cm = confusion_matrix(trues, preds)
    # If binary confusion matrix expected to be 2x2, keep as-is; otherwise print cm directly.
    try:
        TN, FP, FN, TP = cm.ravel()
    except:
        pass

    print("\n===== RESULTS (Prototype + Cosine) =====")
    ...
    history_path = os.path.join(OUTPUT, "history_stage1_prototype.txt")
    os.makedirs(OUTPUT, exist_ok=True)

    with open(history_path, "w") as f:
        f.write("===== STAGE 1 RESULTS (Prototype + Cosine) =====\n")
        f.write(f"Accuracy: {accuracy_score(trues, preds):.4f}\n")
        f.write(f"Precision: {precision_score(trues, preds):.4f}\n")
        f.write(f"Recall: {recall_score(trues, preds):.4f}\n")
        f.write(f"F1-score: {f1_score(trues, preds):.4f}\n\n")
    ...
    # =========================
    # CONFUSION MATRIX HEATMAP (Publication Style)
    # =========================
    class_names = ["Ganwar", "Paddy"]
    cm_array = np.array(cm)

    plt.figure(figsize=(6, 5))
    sns.set_style("white")

    # Professional research-paper colormap
    cmap = "YlGnBu"
